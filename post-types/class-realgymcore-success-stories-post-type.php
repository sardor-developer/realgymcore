...
if ( ! class_exists( 'Realgymcore_Success_Stories_Post_Type' ) ) {
	/**
	 * Post type class
	 *
	 * @return void
	 */
	class Realgymcore_Success_Stories_Post_Type {
		/**
		 * Slug
		 *
		 * @var string
		 */
		private $slug = 'realgym-stories';
		 ...
		 * @var string
		 */
		private $singular_name = 'Success Story';
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_realgymcore_ajax_success_stories_request', array( $this, 'get_ajax_success_stories' ) );
			add_action( 'wp_ajax_nopriv_realgymcore_ajax_success_stories_request', array( $this, 'get_ajax_success_stories' ) );
		}
		...
		public function register() {
			...
			$args     = array(
				'labels'              => $labels,
				'description'         => esc_html__( 'Success Stories', 'realgymcore' ), // Description.
				'supports'            => $supports,
				...
				'rewrite'             => array(
					'slug'       => 'realgymcore-success-stories',
					'with_front' => false,
				),
			);
		...
		public function get_ajax_testimonials() {
			check_ajax_referer( 'realgymcore_success_stories_nonce', 'nonce' );
			$response = array();
		...
		public function register_taxonomy() {
			register_taxonomy(
				'realgym-stories-category',
				$this->slug,
				array(
					'label'        => esc_html__( 'Success Stories category', 'realgymcore' ),
					'hierarchical' => true,
				)
			);
		}
	...
	new Realgymcore_Success_Stories_Post_Type();
}
